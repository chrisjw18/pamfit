...
#' @keywords internal
#' @export 
import_light_steps <- function(path, sep = ',') {
...
#' @keywords internal
#' @export
import_fvfm_files <- function(path, light) {
...
#' output file to (defaults to the directory of the input file).
#' @return list with \code{abso}, \code{x2}, \code{y2}, 
#' \code{total}, and \code{light} values
#' @export
import_data <- function(input_dir = ".", ab = FALSE, output_dir = NULL) {
  # processing data
  light <- import_light_steps(input_dir)
  fv_fm <- import_fvfm_files(input_dir, light)
